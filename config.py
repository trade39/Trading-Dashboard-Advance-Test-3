...
DEFAULT_BENCHMARK_TICKER: str = "SPY" # Default benchmark (e.g., S&P 500 ETF)
...
COLORS: Dict[str, str] = {
    ...
    "gray": "#808080",
    "orange": "#FFA500",
    "purple": "#8A2BE2", # Added a new color for benchmark elements
    "dark_background": "#1C2526",
    "light_background": "#FFFFFF",
    "text_dark": "#E0E0E0",
    "text_light": "#333333",
    "text_muted_color": "#A0A0A0",
    "card_background_dark": "#273334",
    "card_border_dark": "#4169E1",
    "card_background_light": "#F0F2F6",
...
KPI_CONFIG: Dict[str, Dict] = {
    ...
    # ... (other existing KPIs remain here) ...
    "win_rate": {
    ...
    # Add other existing KPIs definitions here... (avg_trade_pnl, avg_win, etc.)
    "avg_trade_pnl": {
    ...
    "avg_loss": {
        ...
        "thresholds": [("Low", 0, 50),("Moderate", 50, 200),("High", 200, float('inf'))],
        "color_logic": lambda v, t: COLORS["red"] if v > 0 else COLORS["gray"]
    },
    "win_loss_ratio": {
    ...
    "max_drawdown_abs": {
        ...
        "thresholds": [("Low", 0, 1000),("Moderate", 1000, 5000),("High", 5000, float('inf'))],
        "color_logic": lambda v, t: COLORS["red"] if v > 0 else COLORS["gray"]
    },
     "var_95_loss": {
    ...
    "pnl_kurtosis": {
        ...
        "color_logic": lambda v, t: COLORS["red"] if v > 1 else COLORS["gray"]
    },
    "max_win_streak": {
    ...
    "benchmark_total_return": {
        "name": "Benchmark Total Return", "unit": "%", "interpretation_type": "neutral", # Higher is better for the benchmark itself
        "thresholds": [("Negative", float('-inf'), 0), ("Positive", 0, float('inf'))],
        "color_logic": lambda v, t: COLORS["green"] if v > 0 else (COLORS["red"] if v < 0 else COLORS["gray"])
    },
    "alpha": {
        "name": "Alpha (Annualized)", "unit": "%", "interpretation_type": "higher_is_better",
        "thresholds": [("Negative Alpha", float('-inf'), 0), ("Positive Alpha", 0, float('inf'))],
        "color_logic": lambda v, t: COLORS["green"] if v > 0 else (COLORS["red"] if v < 0 else COLORS["gray"])
    },
    "beta": {
        "name": "Beta", "unit": "", "interpretation_type": "neutral", # Context-dependent
        "thresholds": [("Low Volatility", 0, 0.8), ("Market Volatility", 0.8, 1.2), ("High Volatility", 1.2, float('inf'))],
        "color_logic": lambda v, t: COLORS["gray"] # Neutral color, interpretation varies
    },
    "benchmark_correlation": {
        "name": "Correlation to Benchmark", "unit": "", "interpretation_type": "neutral",
        "thresholds": [("Negative", -1.0, -0.5), ("Low", -0.5, 0.5), ("Positive", 0.5, 1.01)],
        "color_logic": lambda v, t: COLORS["purple"] # Using a distinct color
    },
    "tracking_error": {
        "name": "Tracking Error (Annualized)", "unit": "%", "interpretation_type": "lower_is_better",
        "thresholds": [("Low", 0, 5), ("Moderate", 5, 15), ("High", 15, float('inf'))],
        "color_logic": lambda v, t: COLORS["green"] if v < 5 else (COLORS["orange"] if v < 15 else COLORS["red"])
    },
    "information_ratio": {
        "name": "Information Ratio", "unit": "", "interpretation_type": "higher_is_better",
        "thresholds": [("Poor", float('-inf'), 0.0), ("Fair", 0.0, 0.5), ("Good", 0.5, 1.0), ("Excellent", 1.0, float('inf'))],
        "color_logic": lambda v, t: COLORS["green"] if v > 0.5 else (COLORS["orange"] if v > 0 else COLORS["red"])
    }
}
...
DEFAULT_KPI_DISPLAY_ORDER: List[str] = [
    ...
    # Benchmark KPIs can be interspersed or grouped
    "benchmark_total_return", "alpha", "beta", "benchmark_correlation", "tracking_error", "information_ratio",
    # Other existing KPIs
    "avg_win", "avg_loss", "win_loss_ratio", "max_drawdown_abs",
...
PLOT_BENCHMARK_LINE_COLOR: str = COLORS["purple"] # Color for benchmark line in plots
